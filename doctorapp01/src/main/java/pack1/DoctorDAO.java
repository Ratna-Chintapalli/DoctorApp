...
public class DoctorDAO {
	...
	public int update(Doctor doctor) {
	    String sql = "update doctor65 set name=?, email=?, specialist=?, city=?, country=? where id=?";
	...
	public int delete(int id) {
	    String sql = "delete from doctor65 where id=?";
	    return jdbcTemplate.update(sql, id);
	}
	
	
	public Doctor getDoctorById(int id) {
	    String sql = "select * from doctor65 where id=?";
	    return jdbcTemplate.queryForObject(sql, new RowMapperImpl(), id);
	}
	
	public List<Doctor> getListOfDoctors() {
	    String sql = "select * from doctor65";
